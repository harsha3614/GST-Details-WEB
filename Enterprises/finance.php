...
$sql_select_auditors = "SELECT * FROM auditors WHERE Enterprise = '$enterprise_name'";
$result_auditors = mysqli_query($conn, $sql_select_auditors);

// Check if the query was successful
if ($result_auditors) {
    // Fetch data from the add_service table
    $sql_select = "SELECT * FROM add_service WHERE Enterprise_Name='$enterprise_name'";
    ...
    if ($result) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if ($row['auditor'] == null) {
                echo "<select class='update-status' data-counseling-id='{$row['id']}' name='auditor_name'>";

                // Reset the internal data pointer to the beginning
                mysqli_data_seek($result_auditors, 0);
                echo "<option>Select</option>";
                // Loop through the result set
                while ($row_auditor = mysqli_fetch_assoc($result_auditors)) {
                    $auditorName = $row_auditor['name'];

                    // No condition for selected, all options are available
                   
                    echo "<option value='$auditorName'>$auditorName</option>";
                }
